import '../styles/MenuDeroulant.scss'
...
function MenuDeroulant ({titre, contenu}) {
    ...
    return (
        ...
        <div className='MenuDeroulantEtContenu'>
        
        <div className="ContainerMenuDeroulant">

            <h2>{titre}</h2>
                        
            <svg width="25" height="14" viewBox="0 0 25 14" fill="none" xmlns="http://www.w3.org/2000/svg" onClick={toggle} className={`fleche ${isOpen ? 'open': ''}`}>
        ...
        <div className={`ContenuMenuDeroulant ${isOpen ? 'open' : 'closed'}`}>{contenu}</div>
